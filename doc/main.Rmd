...
if(run.svm){
  ...
  if(model.selection){
    ...
    if(run.cv){
      ...
      tm_svm_radial_mod < system.time(svm_radial_mod <- svm_radial_train(svm_training_data, 0.1, 1, K))
  ...
  if(run.svm.test){
    ...
    save(tm_svm_linear_test, file="../output/tm_svm_linear_test.RData")
    
    ## performance
    svm_auc
    svm_accu

    cat("The accuracy of svm model is", svm_accu[1]*100, "%.\n")
    cat("The AUC of svm model is", svm_auc[1], ".\n")
  } else {
...
cat("Time for testing svm model=", tm_svm_linear_test[1], "s \n")
```
