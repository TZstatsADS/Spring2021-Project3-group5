...
Visualize cross-validation results. (Has error!)
```{r cv_vis}
...
tm_rfTrain <- system.time(
 rf_fit <- rf_train(dat_train_balanced_rose,mtry = 77)
)
save(rf_fit, file = "../output/train_rf.RD")
```
...
if(run.test){
 ...
 rf_test_pred <- rf_test(model = rf_fit,testdf = dat_test))
}
