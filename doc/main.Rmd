...
run.feature.train <- TRUE # process features for training set. We do not train features on presentation day!
run.test <- TRUE # run evaluation on an independent test set. This should be set to TRUE on presentation day!
run.feature.test <- TRUE # process features for test set
run.poly.feature <- TRUE # process poly features
run.add.poly.feature <- TRUE # and add poly features to dist matrix
run.gbm <- TRUE
needs.balanced <- TRUE # balance data for model fitting
model.selection <- TRUE # perform model selection on svm models
run.balanced.data.ROSE <- FALSE # Whether or not balance the data
run.balanced.data.SMOTE <- TRUE # Whether or not balance the data
...
if (run.presentation.day){
  save(fiducial_pt_list, file="../output/presentation_fiducial_pt_list.RData")
} else {
...
if (run.presentation.day){
  dat_test <- feature(fiducial_pt_list, test_idx, run.poly.feature, run.add.poly.feature)
  save(dat_test, file="../output/presentation_feature_test.RData")
} else {
...
We need to balance our data. We have two methods we can use: SMOTE and ROSE
```{r balance}
if (!run.presentation.day){
  if(run.balanced.data.ROSE){
  ...
  } else if(run.balanced.data.SMOTE) {
    
    dat_train$label <- as.factor(dat_train$label)
    dat_train_balanced_rose <- SMOTE(label ~ ., dat_train, perc.over = 100, perc.under=200)
    #save(dat_train_balanced_rose, file="../output/balanced_data.RData")
    table(dat_train_balanced_rose$label)
...
if (!run.presentation.day){
  ...
  if (run.gbm){
    ...
    if (sample.reweight){
      gbm_tm_train <- system.time(gbm_train <- train_gbm(dat_train_balanced_rose, s=0.1, K=K, n=gbm.numtrees,w = weight_train))
    } else {
      gbm_tm_train <- system.time(gbm_train <- train_gbm(dat_train_balanced_rose, s=0.1, K=K, n=gbm.numtrees,w = NULL))
    }
    ...
    save(best.iter.cv, file="../output/best_trees_gbm_train.RData")
    save(gbm_tm_train, file="../output/gbm_tm_train.RData")
  ...
  load(file="../output/best_trees_gbm_train.RData")
  load(file="../output/gbm_tm_train.RData")
...
if(run.test){
  ...
  if (run.gbm){
    load(file="../output/gbm_train.RData")
    tm_test <- system.time(prob_pred<-test_gbm(gbm_train,as.data.frame(feature_test),n=best.iter.cv,pred.type = 'response'))
    
    label_pred_baseline <- colnames(prob_pred)[apply(prob_pred, 1, which.max)]
...
```{r}
if (run.gbm){
  gbm_accu <- mean(dat_test$label == label_pred_baseline)
  gbm.auc <- WeightedROC(as.numeric(label_pred_baseline), dat_test$label)
