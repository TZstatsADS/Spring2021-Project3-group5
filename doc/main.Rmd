...
gbm.numtrees <- 200 #number of trees to use in gbm
...
if(run.feature.train){
  tm_feature_train <- system.time(dat_train<-feature(fiducial_pt_list,train_idx, run.poly.feature))
...
tm_feature_test <- NA
if(run.feature.test){
  tm_feature_test <- system.time(dat_test <- feature(fiducial_pt_list, test_idx, run.poly.feature))
...
if (run.gbm){
  ...
  print(best.iter.oob)
  best.iter.cv <- gbm.perf(fit_train,method="cv")   # returns 5-fold cv estimate of best number of trees
  print(best.iter.cv)
...
if (run.gbm){
  ...
  cat("The accuracy of model: GBM baseline is", mean(dat_test$label == label_pred)*100, "%.\n")
