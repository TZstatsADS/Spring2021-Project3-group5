...
if(run.feature.train){
  ...
  save(dat_train, file="../output/feature_train_old.RData")
}else{
...
if(run.feature.test){
  ...
  save(dat_test, file="../output/feature_test_old.RData")
}else{
...
if(run.balanced.data){
dat_train_balanced_rose<-ROSE(label~., dat_train,seed=2020)$data
save(dat_train_balanced_rose, file="../output/balanced_train_data_rose_old_feature.RData")
dat_test_balanced_rose <- ROSE(label~., dat_test, seed=2020)$data
save(dat_test_balanced_rose, file = "../output/balanced_test_data_rose_old_feature.RData")
}else{
  load(file = "../output/balanced_train_data_rose_old_feature.RData")
  load(file = "../output/balanced_test_data_rose_old_feature.RData")
}
table(dat_train_balanced_rose$label)
table(dat_test_balanced_rose$label)
...
cat("AUC(balanced) for Random Forest with old feature: ", rf_AUC_balanced,".\n")
cat("Accuracy(balanced) for Random Forest with old feature", rf_accuracy_balanced*100,"%.\n")
cat("Training time (balanced) for Random Forest with old feature: ", time.rf.train.final.balanced[1], "s.\n")
cat("Testing time (balanced) for Random Forest with old feature: ", time.rf.test.final.balanced[1], "s.\n")
cat("   ","\n")
...
cat("AUC(imbalanced) for Random Forest with old feature: ", rf_AUC_imbalanced,".\n")
cat("Accuracy(imbalanced) for Random Forest with old feature", rf_accuracy_imbalanced*100,"%.\n")
cat("Training time (imbalanced) for Random Forest with old feature: ", time.rf.train.final.imbalanced[1], "s.\n")
cat("Testing time (imbalanced) for Random Forest with old feature: ", time.rf.test.final.imbalanced[1], "s.\n")
}
```
...
# Appendix: Tuning the Model
## Tune mtry (balanced Random Forest)
```{r}
...
## Tune Trees and nodes (balanced Random Forest)
```{r}
source("../lib/random_forest.R")
if(tune.random.forest){
  # Train 500 trees:
